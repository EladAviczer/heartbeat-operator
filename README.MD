...
[![CI](https://github.com/EladAviczer/heartbeat-operator/actions/workflows/ci.yaml/badge.svg)](https://github.com/EladAviczer/heartbeat-operator/actions/workflows/ci.yaml)
[![k6 Load Test](https://img.shields.io/github/actions/workflow/status/EladAviczer/heartbeat-operator/load-test.yaml?label=k6%20load%20test&logo=k6&logoColor=white&color=ff5500)](https://github.com/EladAviczer/heartbeat-operator/actions/workflows/load-test.yaml)
...
*   **Exec**: Run arbitrary shell commands and check exit codes.
*   **gRPC**: Check native gRPC health endpoints (`grpc_health_v1`).
*   **DNS**: Verify hostname resolution.
*   **TLS**: Perform TLS handshakes and check certificate expiration (fails < 7 days).
...
To build and run the operator locally for testing:
...
2. **Run It:**
   Ensure you have a valid Kubernetes context configured (e.g., `~/.kube/config`). Because the app connects to the K8s API to watch for `Probe` CRDs and emit Events, it requires an active cluster.
   ```bash
   ./heartbeat-operator
   ```